...
sub pwd_encrypt{
    ...
    $pwd = md5_hex($pwd) if $is_md5_pwd == 0; 
...
sub pwd_encrypt_js {
    ...
    $is_md5_pwd = 1 unless defined $is_md5_pwd;
    my $je;
    if(defined $Webqq::Encryption::_je ){
        $je = $Webqq::Encryption::_je ;
    }
    else{
        my $javascript;
        if(defined $Webqq::Encryption::_javascript){
            $javascript = $Webqq::Encryption::_javascript;
        }
        else{
            local $/ = undef;
            $javascript = <DATA>;
            $Webqq::Encryption::_javascript = $javascript;  
            close DATA;
        }
        $je = JE->new;
        $je->eval($javascript);
        croak "load javascript error: $@\n" if $@;     
        $Webqq::Encryption::_je = $je;
    }   
    ...
    my $p = $je->eval(qq#
        ...
        var r = \$.Encryption.getEncryption(p,salt,verifycode,$is_md5_pwd);
        return(r);
    #);
...
$.Encryption = function() {
    ...
    function getEncryption(password, salt, vcode, isMd5) {
        ...
        TEA.initkey(s2);
        var saltPwd = window.TEA.enAsBase64(rsaH1Len + rsaH1 + TEA.strToBytes(salt) + vcodeLen + hexVcode);
        TEA.initkey("");
        //__monitor(488358, 1);
